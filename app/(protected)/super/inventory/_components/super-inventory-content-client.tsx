...
interface SuperInventoryContentClientProps {
  items: ItemWithOrg[]
  total: number
  page: number
  limit: number
...
export function SuperInventoryContentClient({
  items,
  total,
  page,
  limit,
  ...
  sortOrder = "asc",
  activeFilter,
}: SuperInventoryContentClientProps) {
  ...
  const updateParams = useCallback((updates: Record<string, string | undefined>) => {
    ...
    router.push(`/super/inventory?${params.toString()}`)
  }, [router, searchParams])
          ...
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-xs text-muted-foreground">
                Items will appear here when organizations add them
              </p>
            </div>
          ) : (
...
export function SuperInventoryContentSkeleton() {
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3">
        {[1, 2, 3].map((i) => (
